...
protected:
    ...
    SnakeTest() : snake(RED, KEY_LEFT, KEY_RIGHT, "red") {}
};

TEST_F(SnakeTest, ResetTest) {
    snake.reset();
    EXPECT_EQ(snake.get_trail().size(), 22); // Initial trail is of size 22
}
...
TEST_F(SnakeTest, CheckSelfCollisionTest) {
    EXPECT_FALSE(snake.check_self_collision()); // No self-collision at the start of the game
}
